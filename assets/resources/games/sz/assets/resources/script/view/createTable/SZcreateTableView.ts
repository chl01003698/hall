...
export default class SZcreateTableView extends HallBaseView {
    personality: cc.Node;
    layout: cc.Node;
    createInfo: any;
    ...
    clickBegin() {
        ...
        param.config = SZcreateTableMode.getInstance().getBeginConfig();
    ...
    selectType(isNormal) {
        ...
        if(isNormal) {
            this.updateView(SZConstant.wanfaType.normal);
            SZcreateTableMode.getInstance().setConfigType(SZConstant.wanfaType.normal);
        }else{
            this.isCommonSetting = true;
            this.updateView(SZConstant.wanfaType.quite);
            SZcreateTableMode.getInstance().setConfigType(SZConstant.wanfaType.quite);
        }
    }
    ...
    updateView(type) {
        this.createInfo = SZcreateTableMode.getInstance().getCreateInfoArray(type);
    ...
    _updateWanfaItem(array) {
        ...
        for(var i=0;i<array.length;i++) {
            ...
            if(array[i].selectItem.length > 3) {
                cc.log('games/sz/assets/resources/res/prefab/create/createItemBig');
                prefab = cc.loader.getRes('games/sz/assets/resources/res/prefab/create/createItemBig', cc.Prefab); 
            }else{
                cc.log('games/sz/assets/resources/res/prefab/create/createItem');
                prefab = cc.loader.getRes('games/sz/assets/resources/res/prefab/create/createItem', cc.Prefab); 
            }
            var node = cc.instantiate(prefab);
            var wanfaItem = node.getComponent('SZwanfaItem');
