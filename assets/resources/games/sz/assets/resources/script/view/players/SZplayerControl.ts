...
@ccclass
@menu("sz/SZplayerControl")
export default class SZplayerControl extends cc.Component {
    gameView: any;
    players: any[];     //所有玩家
    ...
    initPlayers(gameView) {
        ...
        for(let i=0;i<this.playersNode.length;i++) {
            var prefab = cc.loader.getRes('games/sz/assets/resources/res/prefab/game/headItem', cc.Prefab) 
            let player = cc.instantiate(prefab).getComponent('SZplayerHead');
    ...
    updateAllScore(data:any) {
        for(let i=0;i<data.length;i++) {
            let localSeatId = SZGameUtil.toLocalSeatId(data[i].sid);
    ...
    */
    ...
    getPlayerPos(index:number) {
        if(index == 0) {
            return this.selfPos.getPosition();
        }
     ...
     * @memberof SZplayerControl
     */
    compaiLoseState(index:number,isShow) {
        this.players[index].compaiLoseState(isShow);
    }
     ...
     * @memberof SZplayerControl
     */
    showPkBtn(index:number,isShow) {
        this.players[index].showPkBtn(index,isShow);
    }
    ...
    hideAllPkBtn() {
        cc.log('hideAllPkBtn');
        for(let i=0;i<this.players.length;i++) {
            cc.log('hideAllPkBtn  222');
            this.players[i].hidePKBtn();
        }
    }
    ...
    * 
    * @param {any} index 
    * @param {any} isGuanzhan 
    * @memberof SZplayerControl
    */
   showGuanzhan(index,isGuanzhan) {
        this.players[index].showGuanzhan(isGuanzhan);
    }
    ...
    hideAllGuanzhan() {
        for(let i=0;i<this.players.length;i++) {
            this.players[i].showGuanzhan(false);
        }
    }
}
