...
@ccclass
@menu("nn/NNbtnItem")
export default class NNbtnItem extends cc.Component {
    isGsp: boolean;
    isSelect: any;
    index: number;
    selectType: any;
    ...
    changCheckBox() {
        ...
        this.text.node.color = this.isSelect ? cc.color(79,221,99) : cc.color(255,255,255);
    }
    ...
    _showMark(defaultSelect:any) {
        ...
        if(defaultSelect.indexOf(this.index) >= 0) {
            ...
            this.text.node.color = cc.color(79,221,99);
        }else {
            this.isSelect = false;
