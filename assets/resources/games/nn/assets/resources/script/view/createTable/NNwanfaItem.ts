...
@ccclass
@menu("nn/NNwanfaItem")
export default class NNwanfaItem extends cc.Component {
    createTable: any;
    select: any;
    selectType: any;
    defaultSelect: any;
    ...
    _updateItem(selectArray) {
        ...
        for (var i = 0; i < selectArray.length; i++) {
            ...
            if (this.createTable.btnItemPool.size() > 0) { // 通过 size 接口判断对象池中是否有空闲的对象
                ...
                var prefab = cc.loader.getRes('games/nn/assets/resources/res/prefab/create/btnItem', cc.Prefab) 
                item = cc.instantiate(prefab);
            }   
            item.parent = this.btnLayout;
            let btnItem = item.getComponent('NNbtnItem');
    ...
    changeSendParam() {
        ...
        NNcreateTableMode.getInstance().setConfigItem(this.playType.name,array);
        NNcreateTableMode.getInstance().setDefaultItem(this.playType.name,this.defaultSelect);
    }

    //改变Default状态
    changeDefaultArray() {
        NNcreateTableMode.getInstance().setDefaultArray(this.playType.name,this.createTable.createInfo.type,this.defaultSelect);
    }
}
