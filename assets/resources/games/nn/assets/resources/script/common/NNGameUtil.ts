import { NNConstant } from "./NNConstant";


export class NNGameUtil {
    //人数
    ...
    //游戏状态
    static game_status: string = NNConstant.gameProgress.table
    ...
    static toLocalSeatId (serverSeatId) {
        cc.log('toLocalSeatId ' + NNGameUtil.playerNum + "  " + NNGameUtil.serverSeatId)
        return NNGameUtil.MAP[NNGameUtil.playerNum][(serverSeatId - NNGameUtil.serverSeatId + NNGameUtil.playerNum) % NNGameUtil.playerNum];
    }

    //本地座位号转服务器桌位号
    static toServerSeatId(localSeatId){
        return ((NNGameUtil.MAP[NNGameUtil.playerNum][localSeatId] - 0) + NNGameUtil.serverSeatId + NNGameUtil.playerNum)%NNGameUtil.playerNum;
    }
