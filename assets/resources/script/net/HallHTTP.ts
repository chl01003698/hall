...
export class HallHTTP {
    ...
    setAuthorization(authorization :any){
        ...
        console.log("setAuthorization", this.authorization) ;
    }

    send(requestType: HallRequestType, url: string, callback: Function, param: any = {}, type: string = null, err:boolean = false) {
        ...
        xhr.onreadystatechange = function () {
            if (xhr.readyState === 4) {
                if (xhr.status >= 200 && xhr.status < 300) {
                    ...
                    h.log.debug("status:" + xhr.status);
                    h.log.debug(JSON.stringify(HallNetConfig.netError));
                }
            }
        };
