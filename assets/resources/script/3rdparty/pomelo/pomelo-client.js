...
(function() {
  ...
  var sendMessage = function(reqId, route, msg) {
    if(useCrypto) {
      ...
      var expiry = Date.now() + timeDifference;
      msg['__expiry__'] = expiry;
      msg['__once__'] = random;
      msg = JSON.stringify(msg);
      console.log("pomelo...useCrypto rsa=" + rsa);
  ...
  var processMessage = function(pomelo, msg) {
    if(!msg.id) {
      // server push message
      pomelo.emit("allPush", msg.route, msg.body);
  ...
  var handshakeInit = function(data) {
    ...
    if(data.time) {
      timeDifference = data.time - Date.now();
    }
