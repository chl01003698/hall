...
export class HallAudioManager {
    ...
    constructor(){    
        ...
        this.musicStatus = h.storage.getItem(this.musicStatusKey,false) || true;
        this.soundVolume = parseFloat(h.storage.getItem(this.soundVolumeKey, false)) || 1;
        this.soundStatus = h.storage.getItem(this.soundStatusKey, false) || true;
        h.log.debug("fuck=== >>>>>>" + this.musicStatus + this.soundStatus);
    ...
    private getUrl(name:string, tag:string = null) {
        ...
        if(tag == HallController.getCurGameName()){
            cc.log('#### sounds' + "resources/" + HallViewConfig.getSearchPath() + "res/sounds/" + path);
